...
export default function TradingAccountsPage() {
  ...
  async function load() {
    try {
      setLoading(true);
      setRows(await api.admin.tradingAccounts.list());
    } catch (e: any) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editingId) await api.admin.tradingAccounts.update(editingId, payload);
      else await api.admin.tradingAccounts.create(payload);
      setOpen(false);
      await load();
    } catch (e: any) {
      setError(e.message);
  ...
  async function handleDelete(id: string) {
    ...
    await api.admin.tradingAccounts.remove(id);
    await load();
  }
